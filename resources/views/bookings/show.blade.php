    ...
    <div class="max-w-5xl mx-auto space-y-5">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="mt-5 flex flex-wrap items-center gap-2">
                ...
                @if ($booking->invoice)
                    <button type="button"
                        @click="$dispatch('open-invoice-preview', { id: {{ $booking->invoice->id }}, number: '{{ $booking->invoice->invoice_number }}', previewUrl: '{{ route('invoices.preview', $booking->invoice, false) }}', downloadUrl: '{{ route('invoices.download', ['invoice' => $booking->invoice, 'filename' => 'Invoice-' . $booking->invoice->invoice_number . '.pdf'], false) }}' })"
                    ...
                    <button type="button"
                        onclick="window.downloadPdfInvoice('{{ route('invoices.download', ['invoice' => $booking->invoice, 'filename' => 'Invoice-' . $booking->invoice->invoice_number . '.pdf'], false) }}', 'Invoice-{{ $booking->invoice->invoice_number }}.pdf')"
                        class="inline-flex items-center gap-1.5 bg-gray-50 text-gray-700 border border-gray-200 px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-100 transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
